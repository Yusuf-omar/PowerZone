document.getElementById('calculate-calories').addEventListener('click', function() {
    ...
    const goal = document.getElementById('goal').value;  // Get goal value

    // Validate inputs
    if (age < 1 || height < 1 || weight < 1 || goal === "") {
        alert("Please enter positive values for age, height, weight, and select a goal.");
        return;
    }
    ...
    // Adjust for the goal
    if (goal === "lose") {
        tdee -= 500;  // 500 calorie deficit to lose weight
    } else if (goal === "gain") {
        tdee += 500;  // 500 calorie surplus to gain weight
    }
